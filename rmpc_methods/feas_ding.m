% FEAS_DING
%
%   Function FEAS_DING evaluates feasibility check of controller design
...
function [chk,chk_eig] = feas_ding(design,model,results,setup)
...
for v = 1 : nv-1 % Minus Nominal System
entity = min(eig(Q{v}));
...
for v = 1 : nv-1 % Minus Nominal System
for vp = 1 : nv-1 % Minus Nominal System
...
mup_verbose(2,vbs,' FEAS_CHK: NOM-VTX:%d: Stability condition matrix MIN_EIG = %x.',v,entity)
if(entity < ZERO);
   cnt = cnt + 1;
   chk(cnt) = 1;
   chk_eig(cnt) = entity;
   mup_verbose(1,vbs,' FEAS_CHK: NOM-VTX:%d: Stability condition violated!',v)
end % if
end % for vp
end % for v
